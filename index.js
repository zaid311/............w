...
// Role position threshold — this role ID and any role higher in hierarchy can use /status
...
// ─── Alert Staff Endpoint ─────────────────────────────────────────────────────
// 1. Sends embed via webhook (with wait=true to get message ID)
// 2. Bot fetches that message and edits it to attach the claim button
...
app.post('/alertstaff', async (req, res) => {
  try {
    ...
    // Step 1 — send via webhook so it looks nice (custom username + avatar)
    ...
    // Step 2 — bot edits the webhook message to add the interactive claim button
    // customId encodes jobId and placeId separated by :: for use in the button handler
...
// Alert claims — one document per claim button press
...
// Playtime sessions — all players, closed when they leave
...
// /status: anyone whose highest role position >= STATUS_MIN_ROLE_ID's position
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId.startsWith('playtime_modal::')) {
      ...
      try {
        ...
        const sessions = await MRSession.find({
        ...
        const recent = [...sessions].sort((a, b) => new Date(b.joinedAt) - new Date(a.joinedAt)).slice(0, 3);
  ...
  if (interaction.isButton()) {
    ...
    if (customId.startsWith('claim_alertstaff::')) {
      // format: claim_alertstaff::<jobId>::<placeId>
      ...
      try {
        ...
        console.error('AlertClaim DB error:', dbErr);
      }

      // Follow-up mention with clickable job ID link
      await interaction.followUp({
    ...
    if (!pending) return interaction.reply({ embeds: [errorEmbed('Confirmation expirée.')], ephemeral: true });
    if (interaction.user.id !== pending.requesterId) return interaction.reply({ embeds: [errorEmbed('Seule la personne qui a lancé la commande peut confirmer.')], ephemeral: true });
    pendingChanges.delete(changeId);

    if (action === 'annuler') return interaction.update({ embeds: [baseEmbed().setDescription('Action annulée.')], components: [] });
    ...
    if (action === 'confirmer') {
      ...
      try {
        ...
        await RankLog.create({
          ...
          oldRank: oldName, newRank: newName,
          reason:  pending.reason || '',
          action:  pending.auditAction,
        });
        await sendLogToChannel(baseEmbed().setDescription('Changement de rang enregistré').addFields(
    ...
    if (action === 'confirmer_ban') {
      ...
      try {
        ...
        if (type === 'unban') {
          ...
          await Ban.create({
            robloxUsername: username.toLowerCase(), robloxUserId: String(userId),
            game, reason, bannedBy: interaction.user.tag, bannedById: interaction.user.id,
          });
          await sendLogToChannel(
  ...
  if (commandName === 'promote') {
    if (!hasPermission(interaction.member)) return interaction.reply({ embeds: [errorEmbed('Permission refusée.')], ephemeral: true });
    const ok = await safeDefer(interaction);
    if (!ok) return;
    try {
      const username = interaction.options.getString('username');
      ...
      const idx = roles.findIndex(r => r.id === oldRole.id);
      if (idx === -1 || idx >= roles.length - 1) return interaction.editReply({ embeds: [errorEmbed(robloxUser.name + ' est déjà au rang le plus élevé.')] });
      const newRole  = roles[idx + 1];
  ...
  if (commandName === 'demote') {
    ...
    try {
      ...
      if (!oldRole) return interaction.editReply({ embeds: [errorEmbed(robloxUser.name + " n'est pas dans le groupe.")] });
      const idx = roles.findIndex(r => r.id === oldRole.id);
      if (idx <= 0) return interaction.editReply({ embeds: [errorEmbed(robloxUser.name + ' est déjà au rang le plus bas.')] });
      const newRole  = roles[idx - 1];
      const changeId = interaction.id + '-' + Date.now();
  ...
  if (commandName === 'ranklog') {
    ...
    if (!total) return interaction.reply({ embeds: [baseEmbed().setDescription('Aucun changement de rang enregistré.')] });
  ...
  if (commandName === 'clearlog') {
    ...
    return interaction.reply({ embeds: [baseEmbed().setDescription('Êtes-vous sûr de vouloir effacer tout le journal ? Action irréversible.')], components: [row], ephemeral: true });
  }
  ...
  // ── /rban ─────────────────────────────────────────────────────────────────────
  if (commandName === 'rban') {
    if (!hasHRPermission(interaction.member)) return interaction.reply({ embeds: [errorEmbed('Permission refusée. Commande réservée aux RH.')], ephemeral: true });
    ...
    try {
      ...
      if (type === 'ban') {
        if (actuallyBanned) return interaction.editReply({ embeds: [errorEmbed(`**${username}** est **déjà banni** du **${gameLabel}**.\n> Raison : ${dbBan?.reason || 'Inconnue'}`)] });
        const changeId = interaction.id + '-' + Date.now();
